...
releases_map_local=/home/cvcms/releases.map
...
if [ $(expr $(date +%H) % 2) -eq 1 ] ; then
   echo INFO $(basename $0) executed every 2 hours.
   exit 0
fi
...
for a in $a_archs ; do
    ...
    [ "x$SCRAM_ARCH" == "x" ] && [ "x$CMSSW_RELEASE" == "x" ] && continue
    break
done

if [ "x$SCRAM_ARCH" == "x" ] ; then
   echo INFO $(basename $0) no arch to install
   exit 0
fi
if [ "x$CMSSW_RELEASE" == "x" ] ; then
   echo INFO $(basename $0) no release to install
   exit 0
fi
...
for f in $files ; do
  ...
  status=$? 
  echo Download status=$status for $f
  if [ $status -ne 0 ] ; then
