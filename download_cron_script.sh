...
for f in $files ; do
  ...
  wget -q -O $workdir/${f}.${host} $weborigin/$f
  status=$?
  [ $status -eq 0 ] || { echo FAILED download for   wget -q -O $workdir/${f}.${host} $weborigin/$f ; exit 1 ; } ;
  diff $workdir/${f}.${host} $workdir/${f} 2>/dev/null 1>/dev/null
  [ $? -eq 0 ] && { echo INFO the downloaded is same. Skipping $f ; continue ; } ;
  echo Warning replacing $f with the downloaded one
  /bin/cp $workdir/${f} $workdir/${f}.${host}_1
  /bin/cp $workdir/${f}.${host} $workdir/${f}
...
files="README.gridpacks README.siteconf README.CVMFS.Variant.Symlink README.goback.publication README.cmssw README.cmssw.jenkins README.cvmfs.manager README.lhapdf"
i=0
for f in $files ; do
   i=$(expr $i + 1)
   echo "[ $i ]" INFO dowloading $f to $workdir/$f
