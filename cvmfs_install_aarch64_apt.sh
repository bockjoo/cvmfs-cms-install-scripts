...
[ \$? -eq 0 ] || { echo apt-get --assume-yes update failed ; echo proot_status=1 ; exit 1 ; } ; \
second_plus= ; \
...
apt-get --assume-yes install cms+cmssw\${second_plus}+$CMSSW_RELEASE > $HOME/POWER8/apt_get_install.${CMSSW_RELEASE}.${SCRAM_ARCH}.log 2>&1 ; \
[ \$? -eq 0 ] || { echo apt-get --assume-yes update failed ; cat $HOME/POWER8/apt_get_install.${CMSSW_RELEASE}.${SCRAM_ARCH}.log ; echo proot_status=1 ; exit 1 ; } ; \
cat $HOME/POWER8/apt_get_install.${CMSSW_RELEASE}.${SCRAM_ARCH}.log ; \
